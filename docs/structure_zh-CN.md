...
├── lib/
│   ├── settings.ts        # 共享 Settings 类型和默认值
│   ├── aria2-rpc.ts       # JSON-RPC 客户端（rpcCall、buildRpcUrl）
│   ├── ariang-url.ts      # AriaNg 面板 URL 构建器
│   └── dom.ts             # DOM 辅助函数（getEl）
├── public/
│   ├── _locales/
...
通过 `browser.runtime.sendMessage` 向 background 发送设置变更。
...
通过 `runtime.sendMessage` 保存设置，由 background 统一处理 RPC 重连逻辑。

## lib/

Entrypoint 引用的共享 TypeScript 模块。这些**不是** entrypoint — WXT 会忽略 `entrypoints/` 之外的所有文件。

| 模块 | 功能 |
|------|------|
| `settings.ts` | `Settings` 接口和 `DEFAULT_SETTINGS` 常量 |
| `aria2-rpc.ts` | `rpcCall()` — 基于 XHR 的 JSON-RPC；`buildRpcUrl()` — URL 构造 |
| `ariang-url.ts` | `buildAriaNgUrl(settings)` — AriaNg 深度链接 URL |
| `dom.ts` | `getEl<T>(id)` — 类型化的 `document.getElementById` |
