...
│   ├── content.ts        # 最小化 Content Script（Cookie 辅助，所有页面注入）
│   ├── popup/
│   │   ├── index.html    # Popup 界面
...
注入所有页面（`<all_urls>`，`document_start`）的最小化 Content Script。目前为 stub，响应 `getCookies` 消息——实际的 Cookie 获取在 background script 中通过 `browser.cookies` 完成。
